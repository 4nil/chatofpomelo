...
var App = function(aSettings, aCanvas) {
	...
	var drawMessages = function(context) {
		...
		for(var i = 0, len = model.messages.length; i<len; i++) {
			model.messages[i].draw(context, 10, context.canvas.height - 50, i);
		}
		model.messages.reverse();
	};
	...
	(function(){
		...
		webSocketService = new WebSocketService(model, pomelo);

		console.log(webSocketService.hasConnection);
	})();
};
